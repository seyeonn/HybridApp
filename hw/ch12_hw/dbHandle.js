...
function createTable() {
   ...
   var createSQL = 'create table if not exists book(type text, name text, author text)';       
 ...
 function insertBook(){ 
    db.transaction(function(tr){
		...
		var author = $('#bookAuthor1').val();
  		var insertSQL = 'insert into book(type, name, author) values(?, ?, ?)';      
     	tr.executeSql(insertSQL, [type, name, author], function(tr, rs){    
...
function updateBook(){
    db.transaction(function(tr){
		...
		var author = $('#bookAuthor2').val();
		var updateSQL = 'update book set type = ?, name = ?, author = ? where name = ?';          
     	tr.executeSql(updateSQL, [type, new_name, author, old_name], function(tr, rs){    
	         console.log('5_책 수정.... ') ;
...
function selectBook2(name){
   db.transaction(function(tr){
	 var selectSQL = 'select type, name, author from book where name=?';        
	   ...
	   $('#bookAuthor2').val(rs.rows.item(0).author);
	 	});
   });         
}
...
function selectBook3(name){
   db.transaction(function(tr){
 	 var selectSQL = 'select type, name, author from book where name=?';      
	   ...
	   $('#bookAuthor3').val(rs.rows.item(0).author);
		}, function(tr, err){
				alert('DB오류 ' + err.message + err.code);
			}
		);
   ...
   db.transaction(function(tr){
 	 var selectSQL = 'select type, name, author from book where name=?';      
  	 tr.executeSql(selectSQL, [name], function(tr, rs){ 
		 ...
		 $('#bookAuthor4').val(rs.rows.item(0).author);
		}, function(tr, err){
				alert('DB오류 ' + err.message + err.code);
			}
		);
