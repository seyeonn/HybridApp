...
function selectBook4(name){
   ...
   var selectSQL3 = 'select * from book where rowid < (select rowid from book where name = ?)';
   ...
   db.transaction(function(tr){
  	...
  	tr.executeSql(selectSQL3, [name], function(tr, rs){ 
   ...
   db.transaction(function(tr){
 	 ...
 	 var selectSQL2 = 'select * from book';      

  	 tr.executeSql(selectSQL2, [], function(tr, rs){ 

       console.log(rs.rows.length);

       listAll =  rs.rows.length;
   ...
   db.transaction(function(tr){
 	 ...
 	 var selectSQL = 'select type, name, author from book where name=?';      
  	 ...
  	 tr.executeSql(selectSQL, [name], function(tr, rs){ 
         ...
         console.log(rs.rows.item(0).rowid);

        $('#indexAll').text(listAll);

        $('#indexThis').text(count+1);
